$(function() {
    ...
    filterContent.forEach((value)=>{
        ...
        value.addEventListener('change', (e)=>{
            ...
            key = e.target.parentElement.nextElementSibling.lastElementChild
            key.value = ''
            while (key.type == 'select-one') {
                if (key.parentElement.nextElementSibling.lastElementChild.type == 'select-one') {
                    if (key.parentElement.nextElementSibling.lastElementChild.name != 'type') {
                ...
                key = key.parentElement.nextElementSibling.lastElementChild
            }

            $.get('../set-adaptive', {'name': name, 'value': value}, function(data){
                if (data['data'] == '500') {
                    e.target.parentElement.nextElementSibling.lastElementChild.disabled = true
                } else {
                    e.target.parentElement.nextElementSibling.lastElementChild.disabled = false
                }
                try {
                    key = e.target.parentElement.nextElementSibling.lastElementChild;
